...
function renderBrowserStatus(status) {
  ...
  elements.browserNotice.textContent = browserConnected
    ? "连接正常。请保持专用 Chrome 打开，确认登录状态后再执行私信。"
    : "尚未连接专用 Chrome。请先运行浏览器，登录抖音后再继续执行。";
  if (currentState) {
    const hasSendableSelection = currentState.tasks.some(
      (task) =>
        task.selected &&
        task.approved &&
        task.executable &&
        task.dmAvailable === true &&
        !task.published,
    );
    elements.executeButton.disabled =
      currentState.running || !browserConnected || !hasSendableSelection;
    elements.locateButton.disabled =
      currentState.running || !browserConnected || currentState.tasks.length === 0;
  }
}
...
function renderTask(task, index) {
  ...
  badges.append(
    makeText("span", "touch-order", `私信 ${index + 1}`),
  ...
  personCell.append(
    ...
    makeText(
      ...
      [
        ...
        `源表 ${task.sourceRows?.length || 1} 行证据`,
      ]
        .filter(Boolean)
        .join(" · "),
    ),
  ...
  profileLink.textContent = task.profileUrl ? "打开公开主页" : "公开主页缺失";
  if (task.profileUrl) {
    profileLink.href = task.profileUrl;
  ...
  commentCell.append(
    ...
    makeText(
      "p",
      "comment",
      task.evidenceComments?.[0] || "评论原文缺失",
    ),
    makeText(
      "span",
      "need",
      `${task.needCategories?.join("、") || "待分类"} · ${task.evidenceCount || 0} 条评论证据`,
    ),
  );

  const replyCell = document.createElement("td");
  const textarea = document.createElement("textarea");
  textarea.value = task.dmText;
  textarea.placeholder = "填写结合用户原话的首条私信";
  textarea.disabled = task.published || currentState.running;
  textarea.addEventListener("change", async () => {
    try {
      await api("/api/update", {
        method: "POST",
        body: JSON.stringify({ id: task.id, dmText: textarea.value }),
      });
...
function render(state) {
  ...
  elements.sourceMeta.textContent = state.sourcePath
    ? `${state.sourceSheet || "工作表"} · ${state.inputRowCount || state.tasks.length} 行评论 → ${state.tasks.length} 个用户 · 合并 ${state.duplicateCount || 0} 行`
    : "等待读取本地 Excel";
  ...
  elements.locateButton.disabled =
    state.running || !browserConnected || state.tasks.length === 0;
  ...
  const hasSendableSelection = state.tasks.some(
    (task) =>
      task.selected &&
      task.approved &&
      task.executable &&
  ...
  elements.executeButton.disabled =
    state.running || !browserConnected || !hasSendableSelection;
...
elements.locateButton.addEventListener("click", async () => {
  try {
    if (!browserConnected) {
      throw new Error("请先运行并连接专用 Chrome");
    }
    const ids = selectedIds();
    if (ids.length === 0) throw new Error("请先勾选要定位的任务");
    ...
    showToast("正在只读打开主页并检查私信入口，不会发送消息。", "info");
    await refresh();
  } catch (error) {
    showToast(error.message, "error");
  }
...
elements.refreshStatusButton.addEventListener("click", async () => {
  try {
    const ids = currentState.tasks
      .filter(
        (task) =>
          task.published || task.status === "verification_pending",
      )
      .map((task) => task.id);
    if (ids.length === 0) throw new Error("当前没有待核验或已发送任务");
    await api("/api/reconcile", {
      method: "POST",
      body: JSON.stringify({ ids }),
    });
    showToast(`正在只读核验 ${ids.length} 条发送记录…`, "info");
    await refresh();
  } catch (error) {
    showToast(error.message, "error");
  }
...
elements.simulateButton.addEventListener("click", async () => {
  try {
    ...
    if (result.ok) {
      showToast(
        `检查通过：${result.executable} 条可执行，${result.published} 条已发送。`,
        "success",
      );
    } else {
...
elements.executeButton.addEventListener("click", async () => {
  try {
    ...
    showToast("私信 worker 已开始执行。", "success");
    await refresh();
  } catch (error) {
    showToast(error.message, "error");
  }
